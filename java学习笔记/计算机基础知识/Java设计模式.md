...
**接口隔离原则（Interface Segregation Principle，ISP）：**客户端不应该被迫依赖于它不使用的方法，一个类对另一个类的依赖应该建立在最小的接口上。接口隔离的优点：
...
建造者模式在应用过程中可以根据需要改变，如果创建的产品种类只有一种，只需要一个具体建造者，这时可以省略掉抽象建造者，也可以省略掉指挥者角色。
...
<img src="../../images/image-20191029164117567.png" alt="image-20191029164117567" style="zoom:67%;" />
...
> Target目标类：Runnable接口
>
> Adaptee适配者：Callable接口
>
> 适配器：FutureTask类
...
* Abstraction：抽象类维护了 Implementor / 即它的实现类 ConcreteImplementorA..， 二者是聚合关系, Abstraction充当桥接类。通过实现类方法调用来完成接口实现。
...
一个类存在两个独立变化的维度，且这两个维度都需要进行扩展时。并且不希望使用继承或因为多层次继承导致系统类的个数急剧增加时使用。这种模式比较少应用，一般可以通过组合接口，实现多种多样扩展行为。
...
<img src="../../images/image-20191109171700162.png" alt="image-20191109171700162" style="zoom:50%;" />
...
* 促销计价场景：】 + 店铺活动 + 店铺优惠券 + 跨店活动 + 平台优惠券  + 会员优惠 + 红包【要求：每个商品享受的活动  + 每个活动参与的品】
...
![image-20191011142506284](../../images/uml_享元设计模式.png)
...
源码使用场景：要String 常量池、Integer/Long cache【缓冲池的场景】
...
<img src="../../images/image-20191108165145326.png" alt="image-20191108165145326" style="zoom:50%;" />
...
<img src="../../images/image-20191109232112925.png" alt="image-20191109232112925" style="zoom:50%;" />
...
算法体现的设计原则：把变化的代码从不变的代码中分离出来；针对接口编程而不是具体类（定义了策略接口）；多用组合/聚合，少用继承（客户通过组合方式使用策略）。

#### 策略模式原理类图

<img src="../../images/image-20191110160212708.png" alt="image-20191110160212708" style="zoom:50%;" />
...
<img src="../../images/image-20191111143228172.png" alt="image-20191111143228172" style="zoom: 50%;" />
...
命令模式应用场景：界面的一个按钮都是一条命令、模拟 CMD（DOS 命令）订单的撤销/恢复、触发反馈机制

源码使用案例：Spring JdbcTemplate#execute

> StatementCallback接口：可以看做**命令接口**
>
> QueryStatementCallback/UpdateStatementCallback等接口：可以看做具体命令实现【没有创建具体接收者类独立实现，而是直接实现命令接口】
>
>  JdbcTemplate#execute(StatementCallback<T> action)：可以看做命令的调用者。
...
<img src="../../images/image-20191111175709843.png" alt="image-20191111175709843" style="zoom:50%;" />
...
适用于复杂业务操作场景。将核心业务的处理拆分到各个链路，每个责任链节点处理一类事情。

责任链缺点：调试不方便，逻辑分散在各个链路；链路过长会对性能有一定影响。

源码使用案例：Dubbo ==> filter机制; Tomcat ==> Value ；SpringMVC 拦截器 ==>HandlerExecutionChain
...
<img src="../../images/image-20191114141829064.png" alt="image-20191114141829064" style="zoom:50%;" />
...
<img src="../../images/image-20191114162125241.png" alt="image-20191114162125241" style="zoom:50%;" />
...
<img src="../../images/3-1Q1161I532V0.png" alt="中介者模式的结构图"  />
...
* Service-manager-rpc/mapper：业务层【service】组合具体manager【rpc,mapper数据库操作】。减少manager依赖manager的情况。
...
<img src="../../images/image-20191115104902105.png" alt="image-20191115104902105" style="zoom:50%;" />
...
<img src="../../images/image-20191108141356173.png" alt="image-20191108141356173" style="zoom: 50%;" />
...
<img src="../../images/image-20191028143615226.png" alt="image-20191028143615226" style="zoom:50%;" />
...
<img src="../../images/image-20191119110248905.png" alt="image-20191119110248905" style="zoom:50%;" />
